...
class tetroromino():
    ...
    def rotate(self):
        ...
        for n, color in enumerate(savetetro):
            self.tetro[(2-(n%4))*4 + (n//4)] = color #cong thuc xoay hinh
...
def DeleteAllRows():
    ...
    for row in range(rows):
        for column in range(columns):
            ...
            grid[0:0] = [0]*columns
            fullrows += 1
    return fullrows**2*100 #xoa hai dong lien tiep nhan doi ddiem
...
while status:
    ...
    screen.blit(textsurface,(wight//2 - textsurface.get_width()//2,5))
    textsurface = pg.font.SysFont('consolas',20).render(f'level:{level}',False,(255,255,255))
    screen.blit(textsurface,(wight//2 - textsurface.get_width()//2,55))
